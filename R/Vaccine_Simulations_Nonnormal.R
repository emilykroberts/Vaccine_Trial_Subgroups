...
library(reshape2); library(tidyr); library(ggplot2); library(pracma)
library(mnormt)
library(lcmix)
array_id = 1
...
alpha1 = 2; psi2 = 0; beta0 = 3; omega1 = 1; beta1 = 4.1; omega2 = 1
eps1 = 1; eps2 = 1; eps3 = 1; tau4 = 0.5; eta4 = 1
theta10<-0.15; theta11<-0.7; thetaT<-0.8
...
#if(method == "E"){eta4 = 0.5; beta1 = beta0; omega1 = -.5; omega2 = 2}
#if(method == "F"){eta4 = 0.5; tau4 = 1; beta1 = beta0; omega1 = -0.5; omega2 = 2}
if(method == "F"){#beta1 = 4; beta0 = 3.5; 
omega1 = -0.75; omega2 = 2}
...
for(i in 2:3){  for(j in 1:(i-1)){    R[i,j]<-R[j,i]  }}
biR = trueR = R
Sig<- S%*%R%*%S
SIG<-S%*%R%*%S
mu<- c(alpha1, beta0, beta1)
X = rnorm(n*iter, mean = eta4, sd = tau4)
X = rbinom(iter*n, 1, 0.5)

sigmae2 = tau = 0.5 # represents ei

if(diffcond == TRUE){
allsamp1 = mvrnorm(iter*n, mu = mu, diag(c(tau, tau, tau)) )
allsamp1[,1] = mu[1]
allsamp1 = allsamp1 + cbind(psi2 * X, omega1 * X, omega2 * X)
uitrue = mvrnorm(iter*n, c(0, 0, 0), biR)

allsamp2 = mvrnorm(iter*n, mu = c(mu[1], mu[2], mu[2]), diag(c(tau, tau, tau)))



allsamp2 = allsamp2 + cbind(psi2 * X, omega1 * X, omega2 * X)

allsamp = cbind(allsamp1, allsamp2[,2:3])
var(allsamp)
...
if(diffcond == FALSE){
  ...
  nu <- 5
  allsampmarg_t = rmt(n*iter, mean = c(alpha1, beta0, beta1), S=R, df=nu)
  hist(allsampmarg_t[,1])
  
  allsamp = allsampmarg_t + cbind(psi2 * X, omega1 * X, omega2 * X)

 allsampmarg_gam = matrix(c(alpha1, beta0, beta1), nrow = n*iter, ncol = 3,
                         byrow = T) + rmvgamma(n*iter, shape=1, rate=1, corr = R)
  allsampmarg_gam = allsampmarg_gam-1
  hist(allsampmarg_gam[,1])
  colMeans(allsampmarg_gam)
  
  allsamp = allsampmarg_t + cbind(psi2 * X, omega1 * X, omega2 * X)
  allsamp = allsampmarg_gam + cbind(psi2 * X, omega1 * X, omega2 * X)

  }

(theta11*sqrt(tau + eps3^2) - theta10*sqrt(tau + eps2^2))/eps1
beta1 - beta0 - alpha1*(theta11*sqrt(tau + eps3^2) - theta10*sqrt(tau + eps2^2))/eps1
...
if(FALSE) uitruesamp = uitrue[setind==set]
ST<-samp; #ST[1:150,1]<-0; ST[1:150,3]<-0; ST[151:300,2]<-0
### only use if baseline measure is included
ST[1:(n/2),1]<-0; ST[(n/2+1):n,2]<-0; ST[1:(n/2),3]<-0
save=ST; if(diffcond == TRUE){ X = samp[,4] # for trt == 0, T11
X[trt == 1] = samp[trt==1,5]
summary(X) }
...
Sigmacoef=matrix((NA),SIM*6,nrow=6)
...
intmarginalizeclosed<-array(0,c((sim-1),1))
slopemarginalizeclosed<-array(0,c((sim-1),1)); slopemarginalize<-array(0,c((sim-1),1))
slopemarginalizewtcl = intmarginalizewtcl = intmarginalize<- (rep(0,1*SIM)); 

holdmu[, 1] = mu; holdmu1[, 1] = holdmu[, 1] + c(psi2, omega1, omega2)
int<-array(0,c((sim-1),1)); int1<-array(0,c((sim-1),1))
holdroc = holdslopeT1 = holdslopeT0 = holdempiricalmean = array(0,c((sim-1),1))

fdeltBeta<-function(n, R, j){  p = 5; q = 6;  a = -0.4 # lower bound of beta
  b = 1 # upper bound of beta
  return( -(n/2) * log(det(R)) + (-0.5 * j) + (q - 1) * log(b - R[2,3]) + # beta
            (p - 1) * log(R[2,3] - a) )}
meanBeta = (6*1 + 3*(-0.4))/(6+3)

holdalpha1[1] = mu[1]; holdbeta0[1] = mu[2]; holdbeta1[1] = mu[3]


# should X be defined counterfactually? or T(0) time 1 for both?
ST = samp[,1:3]
if(TRUE == FALSE){ST[,2:3] = samp[,2:3] - X}
STtrue = ST
...
(cor(ST[,3], ST[,1])*sd(ST[,3]) - cor(ST[,3], ST[,2])*sd(ST[,2]))/sd(ST[,2])
mean(ST[,3]) - mean(ST[,2]) - mean(ST[,1])*(cor(ST[,3], ST[,1])*sd(ST[,3]) - cor(ST[,3], ST[,2])*sd(ST[,2]))/sd(ST[,2])
...
delta3 = mean(ST[,3])
tau4 = sd(X)
phi1 = delta1 - rhoS1X*sigmaS1/tau4 * eta4
phi3 = delta2 - rhoT0X*sigmaT0/tau4 * eta4
phi5 = delta3 - rhoT1X*sigmaT1/tau4 * eta4
phi2 = rhoS1X*sigmaS1/tau4
phi4 = rhoT0X*sigmaT0/tau4
phi6 = rhoT1X*sigmaT1/tau4
xiS1 = sqrt(sigmaS1^2 - rhoS1X^2*sigmaS1^2)
xiT0 = sqrt(sigmaT0^2 - rhoT0X^2*sigmaT0^2)
xiT1 = sqrt(sigmaT1^2 - rhoT1X^2*sigmaT1^2)
psi10 = (rho10*sigmaS1*sigmaT0 - rhoS1X*rhoT0X * sigmaS1 * sigmaT0)/(xiS1*xiT0)
psi11 = (rho11*sigmaS1*sigmaT1 - rhoS1X*rhoT1X * sigmaS1 * sigmaT1)/(xiS1*xiT1)
psiT = (rhoT*sigmaT0*sigmaT1 - rhoT0X*rhoT1X * sigmaT0 * sigmaT1)/(xiT0*xiT1)

(sigmaT1*rho11 - sigmaT0*rho10)/sigmaS1
delta3 - delta2 - (sigmaT1*rho11 - sigmaT0*rho10)/sigmaS1 * delta1
(xiT1 * psi11 - xiT0 * psi10)/xiS1
phi3 - phi2 - (xiT1 * psi11 - xiT0 * psi10)/xiS1 * phi1
 ...
 if(TRUE){ 
  ...
  tmp3<-(ST[trt == 1,3])-XmatS*betaT[,2]
   ...
   if(Xfit == TRUE){
     ...
     theta0S = c(0,0)
     v<-ginv(SIG0Smat+as.numeric(holdS[2,2,sim-1])*(t(Xmat)%*%Xmat))
     ...
     v<-ginv(SIG0Smat+as.numeric(holdS[3,3,sim-1])*(t(Xmat)%*%Xmat))
...
if(Xfit == TRUE){ 
  ...
  eta4h + tau1h^(-1)*tau4h*pi1Xh*(s[n/2] - eta1h)
...
# calc area between curves
#roc = slopeT1 = slopeT0 = rep(NA, n); i=1
#for(i in 1:n){
#  roc[i] = STgathered$value[i + n] - STgathered$value[i];  roc[i] = STgathered$value[i + 3*n] - STgathered$value[i + 2*n]
#  roc[i] =  polyarea(x = c(STgathered$time[STgathered$ID == i]), y = rev(c(STgathered$value[STgathered$ID == i]))[c(1,2,4,3)])
#  slopeT1[i] = STgathered$value[i + 3*n] -  STgathered$value[i + 1*n];  slopeT0[i] = STgathered$value[i + 2*n] -  STgathered$value[i + 0*n]}

#holdroc[sim] = mean(roc); holdslopeT1[sim] = mean(slopeT1); holdslopeT0[sim] = mean(slopeT0)
#holdempiricalmean[sim] = mean(STgathered$value[STgathered$outcome == 1]) - mean(STgathered$value[STgathered$outcome == 0])
#mean(slopeT1 > slopeT0) # should be about zero
print(R)
...
mean(holdroc[2:sim - 1])
mean(holdslopeT1[2:sim - 1])
mean(holdslopeT0[2:sim - 1])
mean(holdempiricalmean[2:sim - 1])
